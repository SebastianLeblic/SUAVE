# aircraft_energy_min_time_climb_template.py
#
...
Setup file to find energy optimal minimum time-to-climb trajectory for given aricraft with constant mass.
...
def main():
    ...
    # search space settings for specific excess power map
    n = 100
    ...
    # get specific excess power map details
    Ps_details = specific_excess_power_range(vehicle, input_details, alt_range, mach_range)
    
    # additional initialization for trajectory profiling
    input_details.init_alt = 500 # units [m]
    input_details.init_mach = 0.1

    # get minimum time-to-climb trajectory
    trajectory_results = minimum_time_to_climb_const_mass(vehicle, input_details)
    ...
    plt.figure('Minimum Time to Climb')
    ...
    plt.title('Minimum Time-to-Climb Trajectory')
    ...
    plt.savefig('Minimum time to climb trajectory.pdf')
    plt.show()
