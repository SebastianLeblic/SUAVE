...
# SU2_inviscid.py
#
# Created:  Jun 23 2016, Sebastian Leblic
# Modified:
...
# ----------------------------------------------------------------------
## @ingroup Analyses-Aerodynamics
class SU2_inviscid_Super_mod(Aerodynamics):
    ...
    def __defaults__(self):
        ...
        self.training = Data()        
        self.training.angle_of_attack  = np.array([-2.,3.,8.,12.]) * Units.deg
        self.training.Mach             = np.array([0.1,0.75,1.1,1.7,2.3])
    ...
    def build_surrogate(self):
        ...
        xy        = training.grid_points        # list of aoa and mach points
        ...
        # regr_cd = gaussian_process._gpr.GaussianProcessRegressor()
        ...
        AoA_points = np.linspace(-5,15,100)*Units.deg
        mach_points = np.linspace(.05,2.05,100)      
        ...
        if plotting == True:
            mappable = plt.cm.ScalarMappable(cmap=plt.cm.viridis)   # added to fix legend (S. Leblic June 23, 2022)
            mappable.set_array(CL_sur)                              # added to fix legend (S. Leblic June 23, 2022)
            fig = plt.figure('Coefficient of Lift Surrogate Plot')    
            plt_handle = plt.contourf(AoA_mesh/Units.deg,mach_mesh,CL_sur,levels=100)
            #plt.clabel(plt_handle, inline=1, fontsize=10)
            cbar = plt.colorbar(mappable)                           # added to fix legend (S. Leblic June 23, 2022)
            # cbar = plt.colorbar()
            ...
            mappable = plt.cm.ScalarMappable(cmap=plt.cm.viridis)   # added to fix legend (S. Leblic June 23, 2022)
            mappable.set_array(CD_sur)                              # added to fix legend (S. Leblic June 23, 2022)
            fig = plt.figure('Coefficient of Drag Surrogate Plot')   
            plt_handle = plt.contourf(AoA_mesh/Units.deg,mach_mesh,CD_sur,levels=100)
            plt.scatter(xy[:,0]/Units.deg,xy[:,1])
            cbar = plt.colorbar(mappable)                           # added to fix legend (S. Leblic June 23, 2022)
            # cbar = plt.colorbar()
            plt.xlabel('Angle of Attack (deg)')
